...
const TESTIMONIALS = [
  { 
    ...
    name: "Daniel Solana", 
    initials: "DS", 
    color: "bg-emerald-50 text-[#24A577]", 
  ...
  { 
    ...
    name: "Raúl Lazcano", 
    initials: "RL", 
    color: "bg-emerald-100 text-[#24A577]", 
  ...
  { 
    ...
    role: "Digital Project Manager Ux & Procesos Sopra Steria (Mercadona)", 
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-heading font-sans text-slate-800 scroll-smooth overflow-x-hidden text-left">
      
      {/* --- NAVEGACIÓN --- */}
      <nav className="sticky top-0 z-50 bg-[#24A577] shadow-md border-b border-white/10 px-4 md:px-6 py-4 text-left">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl overflow-hidden shadow-lg shadow-[#24A577]/20 bg-white shrink-0">
              <img 
                src="https://lh3.googleusercontent.com/d/17ORXUmh_mO98eebsidvJYTDwcBQOYkRU" 
                alt="Logo" 
                className="w-full h-full object-cover"
                onError={(e) => { e.target.src = 'https://placehold.co/100x100/24A577/ffffff?text=JD'; }}
              />
            </div>
          ...
          <a href="mailto:jdemig@gmail.com" className="bg-white text-[#24A577] px-5 py-2 rounded-full text-[11px] font-heading font-bold hover:bg-slate-50 transition-all shadow-lg shrink-0 tracking-wider">Contacto</a>
        </div>
      </nav>

      {/* --- HERO SECTION --- */}
      <header className="bg-gradient-to-r from-[#24A577] to-[#68DBAA] text-white py-12 md:py-32 px-4 md:px-6 relative overflow-hidden text-left">
        ...
        <div className="max-w-7xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8 text-center lg:text-left order-1">
            ...
            <div className="flex flex-col items-center lg:items-start gap-8 pt-4 text-left">
              <div className="flex flex-wrap justify-center lg:justify-start gap-4 text-left">
                <a href="#evidencias" className="flex items-center gap-2 bg-white text-[#24A577] px-8 py-4 rounded-2xl font-heading font-bold hover:bg-slate-50 transition-all shadow-xl shadow-[#24A577]/20 text-sm tracking-wide text-left">
          ...
          <div className="flex flex-col items-center justify-center relative mt-12 lg:mt-0 order-2">
            <div className="w-56 h-56 sm:w-64 sm:h-64 md:w-96 md:h-96 bg-white/10 rounded-[60px] md:rounded-[80px] rotate-0 shadow-2xl flex items-center justify-center border border-white/20 relative overflow-hidden group mb-12 lg:mb-0 backdrop-blur-md">
               <img 
                 src="https://lh3.googleusercontent.com/d/1gAqzgia4mlNYcggujbuolg__MIrUIHIW" 
                 alt="Seal" 
            ...
            <div className="lg:absolute lg:-bottom-6 lg:-left-6 bg-white p-5 md:p-6 rounded-[35px] md:rounded-[45px] shadow-2xl border border-white/50 flex items-center gap-4 animate-in fade-in zoom-in duration-700 text-left">
               <div className="p-3 bg-[#24A577]/10 text-[#24A577] rounded-2xl shadow-inner"><ShieldCheck size={32} /></div>
               <div className="pr-2 text-left">
                 ...
                 <p className="text-xs md:text-sm font-bold text-slate-800 leading-tight font-heading text-left">Product UX & Behavioral Designer<br/><span className="text-[#24A577] font-medium italic">Expert Solutions</span></p>
               </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 -translate-y-8 md:translate-y-[-5rem]">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-24 md:mb-32 text-left">
           {[
             ...
             <div key={i} className="bg-white p-6 md:p-8 rounded-[30px] border border-slate-100 shadow-sm flex items-center gap-5 hover:border-[#24A577]/30 transition-all group text-left">
                <div className="text-[#24A577] group-hover:scale-110 transition-transform">{s.icon}</div>
          ...
          <div className="relative max-w-5xl mx-auto pl-8 md:pl-0">
            ...
            <div className="space-y-16 md:space-y-24 text-left">
              {EXPERIENCE.map((exp, idx) => (
                <div key={idx} className={`relative flex flex-col md:flex-row items-start md:items-center gap-8 md:gap-16 ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                  <div className="absolute left-4 md:left-1/2 w-5 h-5 md:w-6 md:h-6 -translate-x-1/2 bg-white border-4 border-[#24A577] rounded-full z-10 shadow-lg"></div>
                  <div className="w-full md:w-5/12 text-left">
                    <div className="bg-white p-8 md:p-10 rounded-[40px] md:rounded-[50px] shadow-sm border border-slate-100 hover:shadow-2xl transition-all group text-left">
                      <span className="text-[10px] font-heading font-bold text-[#24A577] bg-[#24A577]/5 px-4 py-1.5 rounded-full uppercase mb-5 inline-block tracking-widest text-left">{exp.period}</span>
                      ...
                      <ul className="space-y-3">
                        {exp.bullets.map((b, i) => (
                          <li key={i} className="flex gap-3 text-xs md:text-sm text-slate-600 text-left leading-relaxed font-sans">
                            <CheckCircle2 size={16} className="text-[#24A577] shrink-0 mt-0.5" /> {b}
                          </li>
                        ))}
                      </ul>
                    </div>
            ...
            <div className="flex justify-center mt-20 relative z-10 px-4 text-left">
              <a 
                ...
                target="_blank"
                className="flex items-center gap-3 border border-slate-200 bg-white text-slate-600 px-10 py-5 rounded-[2rem] font-heading font-bold hover:border-[#24A577] hover:text-[#24A577] hover:bg-emerald-50 transition-all shadow-sm text-sm tracking-wide"
              >
        ...
        <section id="evidencias" className="mb-24 md:mb-32 scroll-mt-32 text-left">
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-16 gap-10 text-left">
            ...
            <div className="flex flex-wrap gap-6 justify-start md:justify-end w-full lg:w-auto items-center text-left">
              <a href="https://www.linkedin.com/in/javier-de-miguel-torres/details/experience/urn:li:fsd_profilePosition:(ACoAAAHiTVABe6KeNEe--kkZtNyynUCxBaWlRuA,2252765168)/treasury/" target="_blank" className="flex items-center gap-2 border-2 border-[#24A577] text-[#24A577] bg-white px-6 py-3.5 rounded-2xl font-heading font-bold hover:bg-[#24A577] hover:text-white transition-all text-xs tracking-wider uppercase text-left">lo más actual <ArrowRight size={16} /></a>
              <a href="https://drive.google.com/file/d/1gIM9M-mlU6RepB05VvD0JUcCCaqpzIyu/view?usp=sharing" target="_blank" className="flex items-center gap-2 bg-[#24A577] text-white px-6 py-3.5 rounded-2xl font-heading font-bold hover:bg-[#1a855d] transition-all shadow-lg text-xs tracking-wider uppercase text-left"><FileText size={18} /> Portafolio PDF</a>
              
              <a 
                href="https://drive.google.com/file/d/1ooQJZeeJYsTLYn11CkeaxN24EEf5orbb/view?usp=sharing" 
                target="_blank" 
                className="text-sm font-heading font-bold text-slate-500 hover:text-[#24A577] transition-colors flex items-center gap-2 underline underline-offset-4 decoration-slate-300 hover:decoration-[#24A577] text-left"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 text-left">
            {PROJECTS_PORTFOLIO.map((p, idx) => (
              <div key={idx} className="bg-white rounded-[45px] md:rounded-[55px] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all group flex flex-col h-full border-b-[10px] hover:border-b-[#24A577] relative text-left">
                <div className="h-64 md:h-80 bg-slate-100 relative overflow-hidden text-left">
                  ...
                  {/* --- CAMBIO: Capa verde completa con alpha --- */}
                  <div className="absolute inset-0 bg-[#24A577]/40 transition-opacity duration-300 group-hover:opacity-20"></div>
                ...
                <div className="p-8 md:p-12 flex-1 text-left">
                  ...
                  <h4 className="text-xl md:text-2xl font-heading font-extrabold text-slate-800 mb-4 group-hover:text-[#24A577] transition-colors leading-tight text-left">{p.title}</h4>
                  <p className="text-xs md:text-sm text-slate-500 leading-relaxed mb-8 font-light font-sans text-left">{p.description}</p>
                  <div className="flex flex-wrap gap-2 text-left">{p.tech.map((t, i) => (<span key={i} className="px-3 py-1 bg-[#24A577]/10 text-[#24A577] text-[10px] font-heading font-bold rounded-lg uppercase tracking-tighter">{t}</span>))}</div>
                </div>
                <div className="px-8 md:px-12 pb-8 md:pb-12 text-left">
                   <div className="p-5 bg-[#F8FAFC] rounded-3xl border border-slate-100 flex items-start gap-4 text-left">
                      <Target size={22} className="text-[#24A577] shrink-0 mt-0.5" />
        ...
        <div className="max-w-7xl mx-auto relative text-left">
          ...
          <div className="relative group text-left">
            <button 
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-8 z-20 p-4 bg-white rounded-full shadow-xl text-[#24A577] hover:bg-[#24A577] hover:text-white transition-all opacity-0 group-hover:opacity-100"
            >
              <ChevronLeft size={24} />
            </button>
            <button 
              onClick={() => scrollTestimonials('right')}
              className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-8 z-20 p-4 bg-white rounded-full shadow-xl text-[#24A577] hover:bg-[#24A577] hover:text-white transition-all opacity-0 group-hover:opacity-100"
            >
      ...
      <footer className="bg-[#24A577] py-24 md:py-32 px-4 md:px-6 border-t border-white/5 text-left font-sans text-white">
...
const SectionHeader = ({ title, subtitle }) => (
  <div className="text-left mb-10 md:mb-20">
    <div className="inline-block h-1 w-16 bg-[#24A577] mb-6 rounded-full shadow-[0_0_12px_rgba(36,165,119,0.4)] text-left"></div>
